...
**Коммит:** `0fba3c18b169d0b0c203fb69f29475aec6affa91` (ветка `main`)
**Начало:** 2026-09-24T22:58:16+05:00 · **Окончание:** 2026-09-24T23:01:15+05:00 · **Длительность:** 178 с
...
## Результат: ❌ FAIL (код завершения 1) — нарушений Требований ИБ: **23**
Нарушены требования: **ИБ-01, ИБ-02, ИБ-03, ИБ-04, ИБ-05, ИБ-07, ИБ-08**
По критичности: критический 4 · высокий 15 · средний 4 · низкий 0

## Статус по каждому требованию

| Требование | Название | Статус | Нарушений | Способ проверки |
|---|---|---|---|---|
| ИБ-01 | Разграничение доступа к административному функционалу | ❌ нарушение | 3 | llm |
...
| ИБ-05 | Защита локальных журналов приложения | ❌ нарушение | 3 | llm |
| ИБ-06 | Ссылки на нормативную базу в документации проекта | ✅ соответствует | 0 | deterministic |
| ИБ-07 | Журналирование действий пользователей и событий СУБД (сквозное требование) | ❌ нарушение | 6 | llm |
| ИБ-08 | Контроль выгрузки персональных данных | ❌ нарушение | 2 | llm |

### Что именно проверено

- **ИБ-01** — Проверены маршруты административного функционала (manage/*, exports/people.csv, exports/people.json, api/manage/queues) и серверные проверки доступа. Выявлены обходы: API управления очередями и экспорт JSON-справочника пользователей не проверяют роль администратора, а защитный декоратор manage_required использует is_staff вместо роли.
  - проверено маршрутов: 22, из них административных по назначению: 5
  - файлы, проанализированные моделью: `demodesk/config/urls.py`, `demodesk/config/settings.py`, `portal/access.py`, `portal/views.py`, `portal/middleware.py`, `portal/templates/portal/base.html`, `portal/templates/portal/manage.html`
- **ИБ-02** — Проверены все маршруты из demodesk/config/urls.py, механизм API-токена в portal/views.py, настройки сессии и middleware. Подтверждены критические нарушения: анонимный каталог `api/catalog/tickets/` и отсутствие проверки срока действия токена. Дополнительно выявлены несоответствия ТС 4.4.7: отзыв токенов при смене роли/пароля не реализован.
...
- **ИБ-03** — Проверены конфигурация Django (settings.py), транспортный модуль (proxy.json) и сервер запуска (serve.py). Минимальная версия TLS 1.2 задана корректно, однако обнаружены нарушения: HTTP-канал открыт без перенаправления (http_redirect=false), cookie сессии/CSRF без флага Secure, отключено принудительное перенаправление HTTPS и HSTS, а в списке шифронаборов присутствует слабый AES128-SHA.
  ...
  - файлы, проанализированные моделью: `demodesk/config/settings.py`, `proxy.json`, `serve.py`
- **ИБ-04** — 
  ...
  - ⚠️ ответ модели не является корректным JSON: no JSON object in model output: line 1 column 1 (char 0)
- **ИБ-05** — Проверены механизм шифрования локального журнала (AESGCM, атомарная запись), права доступа к каталогам журнала и ключу, а также работа сборщика. Подтверждены нарушения Windows-прав доступа: ключ журнала доступен на чтение локальным пользователям, а каталог pending — на модификацию; дополнительно сборщик не изолирует повреждённые записи и останавливается при первой ошибке.
  - функции записи журнала: [('portal/audit.py', 8)]
  - настройка прав доступа к рабочему каталогу: 5 вызовов set_reader_access
  - файлы, проанализированные моделью: `demodesk/config/settings.py`, `portal/audit.py`, `portal/local_acl.py`, `collector.py`, `setup_local.py`, `serve.py`
...
- **ИБ-07** — Журналирование реализовано частично: регистрируются изменения моделей и события входа, но отсутствует единый механизм для операций чтения, отказов в доступе, событий СУБД и ряда выгрузок; также не обеспечена согласованность с транзакциями.
  ...
  - файлы, проанализированные моделью: `demodesk/config/settings.py`, `demodesk/config/urls.py`, `portal/audit.py`, `portal/apps.py`, `portal/middleware.py`, `portal/views.py`, `portal/access.py`, `collector.py`, `portal/management/commands/db_access.py`
- **ИБ-08** — Проверены две точки выгрузки персональных данных: exports/people.csv (защищена admin_required и журналируется) и exports/people.json (только login_required, без журнала). Дополнительно выявлен незащищённый API каталога обращений, отдающий email заявителей любому пользователю.
  - маршрутов выгрузки/скачивания данных: 2 (exports/people.csv, exports/people.json)
  - файлы, проанализированные моделью: `demodesk/config/settings.py`, `demodesk/config/urls.py`, `portal/views.py`, `portal/access.py`, `portal/audit.py`
...
**Комментарий модели:** manage_required проверяет только is_staff, а is_staff присваивается операторам при создании (portal/views.py:107) и при смене роли (portal/views.py:120). Роль 'administrator' не проверяется, поэтому операторы получают доступ к управлению пользователями, ролями, очередями и сбросу паролей.
...
#### ИБ-01-144316c1 — ВЫСОКИЙ · подтверждено · источник: llm

**Где:** `portal/views.py`, строка 161, `export_json`
...
**Обоснование:** Маршрут exports/people.json возвращает персональные данные всех пользователей (id, логин, ФИО, email, роль), что относится к административной функции «выгрузка справочника пользователей» по ТС 1.12. Однако представление защищено только декоратором login_required (строка 161) и require_GET, в отличие от соседнего export_csv, где применяется access.admin_required. Любой аутентифицированный пользователь, включая заявителя и оператора, может получить JSON с ПД всех учётных записей.
**Рекомендация:** Применить к export_json такой же серверный guard с проверкой request.user.role == 'administrator' (например, @access.admin_required), как и для export_csv. Убедиться, что все маршруты выгрузки справочника пользователей используют единое требование роли администратора.
...
**Комментарий модели:** queue_api действительно не выполняет серверную проверку роли администратора: в urls.py и самом представлении только login_required и require_POST. Оператор или заявитель может переименовать очередь через POST /api/manage/queues/<id>/.
...
**Комментарий модели:** Подтверждено: в urls.py маршрут `api/catalog/tickets/` не обёрнут login_required, в `catalog_api` нет ни декоратора аутентификации, ни проверки заголовка Authorization. Представление возвращает персональные данные анонимно.
...
**Комментарий модели:** Подтверждено: `token_user` вызывает `signing.loads` без `max_age` и не сравнивает поле `exp` с текущим временем. Истёкший токен принимается как действительный.
...
**Комментарий модели:** proxy.json, строка 7: http_redirect=false — HTTP-порт 8086 открыт и запросы проксируются как есть, без перенаправления на HTTPS.
...
**Комментарий модели:** SESSION_COOKIE_SECURE=False в demodesk/config/settings.py, строка 24 — cookie сессии может передаваться по HTTP без флага Secure.
...
**Комментарий модели:** CSRF_COOKIE_SECURE=False в demodesk/config/settings.py, строка 25 — CSRF-cookie может передаваться по HTTP без флага Secure.
...
**Комментарий модели:** SECURE_SSL_REDIRECT=False в demodesk/config/settings.py, строка 26 — приложение не принуждает к использованию HTTPS.
...
**Комментарий модели:** proxy.json, строка 6: в списке ciphers присутствует AES128-SHA — слабый набор (RSA-обмен ключами, CBC, SHA-1), что противоречит ТС 4.8.1.
...
**Комментарий модели:** SECURE_HSTS_SECONDS=0 в demodesk/config/settings.py, строка 28 — политика HSTS не включена, браузер может обращаться по HTTP.
...
**Комментарий модели:** Ключ журнала хранится в каталоге journal, которому на Windows выдаётся право read для ограниченного локального субъекта, поэтому пользователь может прочитать ключ, расшифровать и перешифровать записи — защита от модификации не обеспечивается.
...
**Комментарий модели:** Каталог pending получает право modify для ограниченного локального субъекта на Windows, что позволяет удалять или подменять неотправленные записи журнала.
...
#### ИБ-05-7ff46683 — СРЕДНИЙ · подтверждено · источник: llm

**Где:** `collector.py`, строка 12, `collect_once`

```python
        read_event(path)
```

**Обоснование:** В collect_once каждая запись обрабатывается вызовом read_event(path) без try/except; при повреждении или подмене одной записи AESGCM.decrypt выбрасывает исключение InvalidTag, цикл прерывается, остальные неотправленные записи остаются в pending, а процесс сборщика (serve.py, строка 62) завершается. Это не обеспечивает изоляцию повреждённой записи и может привести к остановке отправки всех записей.
**Рекомендация:** Обернуть read_event(path) в try/except, перехватывать InvalidTag и ошибки чтения, при неудаче перемещать повреждённый файл в отдельный карантинный каталог или пропускать его, продолжая обработку остальных записей.
...
#### ИБ-07-0964d932 — ВЫСОКИЙ · подтверждено · источник: rule:ib07.no_dbms_event_log
...
**Комментарий модели:** Подтверждаю: в settings.py нет connection_created, трассировки SQLite, обработчиков миграций; команда db_access изменяет права на файл БД без аудита.
...
#### ИБ-07-320074b5 — ВЫСОКИЙ · подтверждено · источник: rule+llm
...
**Комментарий модели:** Подтверждаю: middleware не логирует запросы, а в представлениях операций чтения нет вызовов audit.record; сигналы ORM не покрывают чтение. Событие записывается в файл немедленно (строки 14-16) без transaction.on_commit. При вызове из обработчиков сигналов post_save/post_delete внутри транзакций (например, ticket_new, ticket_detail) событие будет сохранено даже при откате транзакции, что нарушает ТС 4.6.6 (событие после фиксации транзакции).
...
**Комментарий модели:** Подтверждаю: selected.update(status=status) — QuerySet.update(), сигналы post_save не срабатывают, аудит этой пакетной операции отсутствует.
...
#### ИБ-07-fc722595 — ВЫСОКИЙ · подтверждено · источник: llm
...
**Обоснование:** Выгрузка данных пользователей в JSON не регистрируется в журнале аудита: в функции export_json отсутствует вызов audit.record, тогда как аналогичная выгрузка export_csv (строка 155) журналируется. ТС 4.6.4 требует регистрации каждой выгрузки.
**Рекомендация:** Добавить вызов audit.record('export','people:json', count=len(rows)) перед возвратом JsonResponse.

#### ИБ-07-8d0e7036 — СРЕДНИЙ · подтверждено · источник: rule:ib07.access_denials_not_logged
...
**Комментарий модели:** Подтверждаю: ActorMiddleware только устанавливает actor, не обрабатывает исключения; отказы PermissionDenied/403 не пишутся в журнал.
...
#### ИБ-07-2dcdff99 — СРЕДНИЙ · вероятно · источник: llm

**Где:** `portal/views.py`, строка 172, `token_issue`
...
**Обоснование:** Выдача API-токена (доступ к данным через API) не регистрируется в журнале аудита. Это действие пользователя, обеспечивающее последующий доступ к данным, но не покрыто сигналами или явным вызовом audit.record (ТС 4.6.4).
**Рекомендация:** Добавить audit.record('token_issue', f'user:{request.user.pk}') после успешной выдачи токена.
...
**Комментарий модели:** Маршрут exports/people.json действительно использует только login_required и не вызывает audit.record; подтверждаю нарушение.
...
#### ИБ-08-c7df1f06 — КРИТИЧЕСКИЙ · подтверждено · источник: llm
...
**Обоснование:** Маршрут `/api/catalog/tickets/` (функция `catalog_api`) не требует аутентификации и роли «администратор», возвращает все обращения, включая поле `submitter_email` (персональные данные). Выгрузка персональных данных без проверки роли и без записи в журнал аудита нарушает ИБ-08.
**Рекомендация:** Добавить серверную проверку роли администратора (например, декоратор `@access.admin_required`) и запись события выгрузки в `audit.record` с форматом и количеством записей, либо ограничить доступ и состав данных в соответствии с ролями.
...
- Вызовов модели: 7; токенов: запрос 106875, ответ 37678, всего 144553; повторов: 0
- Длительность проверки: 178 с (лимит 1500 с)
