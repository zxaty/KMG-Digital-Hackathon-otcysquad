...
**Коммит:** `de7017c3188b665480961742738a2498bb830f7a` (ветка `main`)
**Начало:** 2026-09-25T09:58:20+05:00 · **Окончание:** 2026-09-25T09:58:45+05:00 · **Длительность:** 24 с
...
## Результат: ❌ FAIL (код завершения 1) — нарушений Требований ИБ: **19**
Нарушены требования: **ИБ-01, ИБ-02, ИБ-03, ИБ-04, ИБ-05, ИБ-07, ИБ-08**
По критичности: критический 3 · высокий 14 · средний 2 · низкий 0
...
| ИБ-07 | Журналирование действий пользователей и событий СУБД (сквозное требование) | ❌ нарушение | 4 | llm |
...
- **ИБ-02** — Проверка сессии и токена на стороне сервера реализована не полностью: большинство маршрутов защищены login_required, но API-маршруты api/v1/tickets/ и api/catalog/tickets/ не имеют серверной проверки сессии или токена; токен API не проверяет срок действия (exp).
...
- **ИБ-03** — Проверены настройки Django (settings.py), конфигурация транспортного модуля (proxy.json) и код запуска (serve.py). Обнаружены нарушения: отключено принудительное перенаправление на HTTPS, cookie без флага Secure, HSTS не настроен, в списке шифронаборов присутствует слабый AES128-SHA.
...
- **ИБ-04** — Пароли хешируются Argon2 с явными параметрами стоимости, вложения шифруются AES-GCM с ключом 256 бит, однако персональные данные (ФИО, логин, email) в модели User хранятся в открытом виде без шифрования полей или шифрованной СУБД.
...
- **ИБ-05** — Локальные журналы шифруются AES-GCM с уникальным nonce и атомарной записью, но ключ журнала хранится в каталоге journal, доступном на чтение непривилегированным пользователям, а каталог pending доступен на изменение, что позволяет подменять записи до отправки.
...
#### ИБ-01-213af066 — ВЫСОКИЙ · подтверждено · источник: rule+llm
...
**Комментарий модели:** Подтверждаю: manage_required проверяет is_staff, который присваивается операторам, что даёт им доступ к административным функциям. Декоратор manage_required, используемый для защиты административных маршрутов manage/, manage/users/<int:user_id>/role/, manage/users/<int:user_id>/queue/, manage/users/<int:user_id>/password/, проверяет только request.user.is_staff. В проекте is_staff присваивается всем пользователям с ролью, отличной от 'applicant' (см. portal/views.py строки 107 и 120), включая операторов. Таким образом, операторы получают доступ к административному функционалу, что нарушает требование о предоставлении доступа исключительно роли 'administrator'.
...
**Обоснование:** Маршрут exports/people.json (export_json) выгружает справочник пользователей с персональными данными (ФИО, email, роль), что является административной функцией согласно ТС 1.12. Однако представление защищено только декораторами login_required и require_GET, без проверки роли 'administrator'. Это позволяет любому аутентифицированному пользователю, включая заявителей и операторов, получить доступ к персональным данным всех пользователей.
**Рекомендация:** Применить к представлению export_json серверную проверку роли администратора (например, декоратор admin_required) до формирования и возврата данных.
...
**Комментарий модели:** http_redirect = false в proxy.json, строка 7.
...
**Комментарий модели:** SESSION_COOKIE_SECURE = False в settings.py, строка 24.
...
**Комментарий модели:** CSRF_COOKIE_SECURE = False в settings.py, строка 25.
...
**Комментарий модели:** SECURE_SSL_REDIRECT = False в settings.py, строка 26.
...
**Комментарий модели:** В списке ciphers присутствует AES128-SHA (proxy.json, строка 6).
...
**Комментарий модели:** SECURE_HSTS_SECONDS = 0 в settings.py, строка 28.
...
**Комментарий модели:** Подтверждаю: модель User хранит ПД в открытом виде, шифрование полей отсутствует. Поля username, first_name, last_name, middle_name, email объявлены как обычные CharField/EmailField без шифрования. В settings.py не настроена шифрованная СУБД (используется SQLite). Это нарушает ТС 4.5.3: структурированные ПД должны храниться с аутентифицированным шифрованием с ключом ≥ 256 бит.
...
**Комментарий модели:** Подтверждаю: ключ журнала хранится в каталоге journal, доступном на чтение непривилегированным субъектам. Каталог journal, в котором хранится ключ шифрования журнала (LOG_KEY_FILE = RUNTIME / 'journal' / 'journal.key'), получает право read для непривилегированных локальных субъектов. Зная ключ, пользователь может расшифровать, изменить и заново зашифровать записи журнала, что нарушает требование защиты от модификации (ТЗ 4.5.5, ТС 4.6.7). Каталог pending, содержащий неотправленные записи журнала, получает право modify для непривилегированных локальных субъектов. Это позволяет удалять или подменять записи до их передачи сборщику, что нарушает требование защиты от модификации (ТС 4.6.7).
...
**Комментарий модели:** Подтверждаю: журнал событий СУБД отсутствует. Журнал событий СУБД не реализован: нет обработчиков connection_created, трассировки SQLite, регистрации миграций или изменения прав на файл БД. ТС 4.6.5 требует вести журнал событий СУБД.
...
**Комментарий модели:** Подтверждаю: операции чтения не регистрируются ни middleware, ни в представлениях. [portal/views.py:22] Операция чтения списка обращений не регистрируется: в представлении нет вызова audit.record, middleware не перехватывает GET-запросы. ТС 4.6.4 требует регистрировать просмотр списков. [portal/views.py:41] Просмотр карточки обращения не регистрируется: отсутствует вызов audit.record, middleware не логирует чтение. ТС 4.6.4 требует регистрацию просмотра карточек. [portal/views.py:85] Скачивание вложения не регистрируется: нет вызова audit.record, middleware не логирует чтение. ТС 4.6.4 требует регистрацию скачивания вложений. [portal/views.py:185] Чтение через API не регистрируется: в представлении нет вызова audit.record, middleware не логирует GET-запросы. ТС 4.6.4 требует регистрацию чтения через API. [portal/views.py:202] Чтение каталога обращений через API не регистрируется: нет вызова audit.record, middleware не логирует чтение. ТС 4.6.4 требует регистрацию чтения через API.
...
**Комментарий модели:** Подтверждаю: пакетное обновление через QuerySet.update() не вызывает сигналы и не регистрируется. Пакетное обновление статусов через QuerySet.update() не вызывает сигналы post_save, поэтому не регистрируется. ТС 4.6.4 требует регистрацию пакетных операций с указанием затронутых объектов.
...
**Комментарий модели:** Подтверждаю: отказы в доступе не регистрируются. Отказы в доступе (PermissionDenied/403) не регистрируются: middleware не обрабатывает исключения, нет записи события отказа. ТС 4.6.4 требует регистрацию отказов в доступе.
...
**Комментарий модели:** Подтверждаю: export_json не имеет проверки роли администратора и не пишет аудит. Функция export_json выгружает персональные данные пользователей через people_rows(), но не имеет декоратора admin_required (только login_required) и не вызывает audit.record. Это нарушает требование ИБ-08: выгрузка доступна любому аутентифицированному пользователю, факт выгрузки не фиксируется.
...
- Вызовов модели: 7; токенов: запрос 106322, ответ 5789, всего 112111; повторов: 0
- Длительность проверки: 24 с (лимит 1500 с)
